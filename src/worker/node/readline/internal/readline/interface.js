...
const { addAbortListener } = require('../events/abort_listener');
const {
  charLengthAt,
  charLengthLeft,
  commonPrefix,
...
function InterfaceConstructor(input, output, completer, terminal) {
  ...
  if (!this.terminal) {
    ...
    emitKeypressEvents ??= require('./emitKeypressEvents');
...
class Interface extends InterfaceConstructor {
  ...
  [SymbolAsyncIterator]() {
    if (this[kLineObjectStream] === undefined) {
      kFirstEventParam ??= require('../events/symbols').kFirstEventParam;
