self.onmessage = async (ev) => {
	...
	console.log = (...args) => { origConsole.log(...args); self.postMessage({ type: 'log', text: formatArgs(args) }); };
	console.error = (...args) => { origConsole.error(...args); self.postMessage({ type: 'error', text: formatArgs(args) }); };
	console.warn = (...args) => { origConsole.warn(...args); self.postMessage({ type: 'warn', text: formatArgs(args) }); };
	console.info = (...args) => { origConsole.info(...args); self.postMessage({ type: 'log', text: formatArgs(args) }); };
	...
	try {
		...
		const fn = new Function('fs', 'buffer', 'path', 'events', 'stream', 'util', 'zlib', asyncBody);
		const result = await fn(fs, buffer, path, events, stream, util, zlib);
		const text = result !== undefined ? (typeof result === 'string' ? result : JSON.stringify(result, null, 2)) : undefined;
		self.postMessage({ type: 'result', text });
	} catch (e) {
		self.postMessage({ type: 'runtime-error', message: e.message, stack: e.stack });
	} finally {
		// restore console
